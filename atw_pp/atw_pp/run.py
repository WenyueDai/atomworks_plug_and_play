...
from .io.cif import list_structures, safe_parse_structure
...
def build_metadata(
    ...
    *,
    ...
    print(f"Found {len(paths)} CIF files under: {cif_dir}")
    ...
    for path in paths:
        ...
        aa = safe_parse_structure(path, assembly_id=cfg.assembly_id)
        if aa is None:
            bad_rows.append(
                attach_params(
